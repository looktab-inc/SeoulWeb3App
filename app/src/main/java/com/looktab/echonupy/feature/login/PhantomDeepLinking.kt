...
interface PhantomDecryptor {
    fun decryptPayload(
        data: String,
        nonce: String,
        sharedSecretDapp: ByteArray
    ): Result<ByteArray, PhantomError>
}
...
sealed class PhantomResponse {
    data class OnConnect(
        val response: PhantomConnectResponseData,
        val sharedSecretDapp: String,
        val phantomEncryptionPublicKey: String
    ) : PhantomResponse()
}
...
class PhantomDeepLink constructor(
    _urlSchema: String,
    _appUrl: URL,
    _publicKey: ByteArray,
    _secretKey: ByteArray,
    _cluster: Cluster
) {
    ...
    fun getConnectURL(): String {
        ...
        queryItems["redirect_link"] =
            URLEncoder.encode("${urlSchema}://${PhantomAction.connect.host()}", "utf-8")
    ...
    fun parserHandleURL(uri: Uri): Result<PhantomResponse, PhantomError> {
        ...
        if (errorCodeQueryParam != null && errorMessageQueryParam != null) {
            return Result.Failure(
                PhantomError.ErrorResponse(
                    PhantomErrorResponse(
                        errorCodeQueryParam,
                        errorMessageQueryParam
                    )
                )
            )
        }
        ...
        when (section) {
            PhantomAction.connect.host() -> {
                val phantomEncryptionPublicKey =
                ...
                val sharedSecretDapp = when (val sharedKeyResult =
                    generateSharedSecretDapp(phantomEncryptionPublicKey)) {
                    is Result.Success -> sharedKeyResult.value
                    is Result.Failure -> return Result.Failure(sharedKeyResult.reason)
                }

                val decryptedData = when (val decryptPayloadResult =
                    decryptor.decryptPayload(data, nonce, sharedSecretDapp)) {
                ...
                val phantomConnectResponseData = PhantomConnectResponseData(
                    decryptedJson.get("public_key").toString(),
                    decryptedJson.get("session").toString()
                )

                return Result.Success(
                    PhantomResponse.OnConnect(
                        phantomConnectResponseData,
                        Base58.encode(sharedSecretDapp),
    ...
    private fun generateSharedSecretDapp(phantomEncryptionPublicKey: String): Result<ByteArray, PhantomError> {
        return try {
            val box =
                TweetNaclFast.Box(Base58.decode(phantomEncryptionPublicKey), dappKeyPair.secretKey)
    ...
    fun storeSession(
        ...
        sharedSecretDapp: String
    ) {
        val preferences = context.getSharedPreferences(
            PhantomLoginFragment.SESSION_SHARED_PREFS_FILE,
        ...
        editor.putString(
            PhantomLoginFragment.PHANTOM_ENCRYPTION_PUBLIC_KEY,
            phantomEncryptionPublicKey
        )
